...
import DateDetailContainer from '../organism/DateDetailContainer';
...
function Calendar({
  ...
  const [clickedDay, setClickeDay] = React.useState(0);
    ...
    <>
      <DayPicker
        ...
        classNames={{
          ...
          cell: 'text-center relative [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 hover:text-cyan',
        ...
        components={{
          ...
          DayContent: ({ date }) => {
            ...
            if (!price) {
              return (
                <span className={nowDay === day ? 'text-golden' : ''}>
                  {day}
                </span>
              );
            }
